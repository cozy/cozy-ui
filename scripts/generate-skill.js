...
function parseReadme(filePath) {
  ...
  return { description: shortDescription, allExamples, importPath }
}
...
function hasBalancedDelimiters(text) {
  ...
  return stack.length === 0 && !inString && !inBlockComment
}

function formatCodeBlock(imports, render) {
  const lines = render.split('\n')
  if (lines.length > MAX_EXAMPLE_LINES) return null
  if (!hasBalancedDelimiters(render)) return null
  return [...imports, '', render].join('\n')
}
...
function extractFromBlock(example) {
  ...
  if (semicolonIdx !== -1) {
    ...
    if (STYLEGUIDIST_PATTERNS.some(re => re.test(render))) return null
    return { imports: cleanImports, render }
  }
...
function trimExample(allExamples) {
  ...
  for (const example of allExamples) {
    ...
    if (result) {
      const formatted = formatCodeBlock(result.imports, result.render)
...
function getPrimaryImport(component) {
  const { name, readmeParsed, relativePath } = component
  const resolvedImportPath = `cozy-ui/transpiled/react/${relativePath}`

  for (const example of readmeParsed.allExamples) {
    const allImports = [
      ...example.matchAll(
        /import\s+([A-Za-z_$][\w$]*)\s+from ['"]cozy-ui\/transpiled\/react\/([^'"]+)['"]/g
      )
    ]
    const exactPathImport = allImports.find(m => m[2] === relativePath)
    if (exactPathImport) {
      return `import ${exactPathImport[1]} from '${resolvedImportPath}'`
    }
  }

  return `import ${name} from '${resolvedImportPath}'`
}

function generateComponentSection(component) {
  const { name, readmeParsed, props, relativePath } = component

  const importStatement = getPrimaryImport(component)
  const sectionTitle = relativePath === name ? name : `${name} (${relativePath})`
...
function generateComponentsMd(byCategory) {
  ...
  for (const category of CATEGORY_ORDER) {
    ...
    const links = components
      .map(component => {
        const title =
          component.relativePath === component.name
            ? component.name
            : `${component.name} (${component.relativePath})`
        const anchor = title
          .toLowerCase()
          .replace(/[`]/g, '')
          .replace(/[^a-z0-9 -/()]/g, '')
...
function main() {
  ...
  fs.writeFileSync(path.join(REFERENCES_DIR, 'components.md'), componentsMd)
  console.log(`  ✓ ${path.relative(ROOT, path.join(REFERENCES_DIR, 'components.md'))}`)

  // Copy static css-classes.md
  const cssClassesSrc = path.join(DOCS_SKILL_DIR, 'css-classes.md')
  if (fs.existsSync(cssClassesSrc)) {
    fs.copyFileSync(cssClassesSrc, path.join(REFERENCES_DIR, 'css-classes.md'))
    console.log(`  ✓ ${path.relative(ROOT, path.join(REFERENCES_DIR, 'css-classes.md'))}`)
  } else {
