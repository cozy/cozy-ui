// Automatically created, please run `scripts/generate-svgr-icon.sh assets/icons/ui/walk.svg` to regenerate
import React from 'react'

import SvgWalkTwake from './Walk_twake'
import { isTwakeTheme } from '../helpers/isTwakeTheme'

function SvgWalk(props) {
  if (isTwakeTheme()) return SvgWalkTwake(props)
  return (
    <svg viewBox="0 0 16 17" {...props}>
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M10.602 3.663A2 2 0 118.38.337a2 2 0 012.222 3.326zm-.121 3.73a2.55 2.55 0 001.51.607 1 1 0 110 2 4.071 4.071 0 01-3-1.29l-.69 1.58 2.33 1.94a1 1 0 01.36.77v2a1 1 0 01-2 0v-1.53l-2.58-2.12-1.48 4a1.003 1.003 0 11-1.88-.7L5.87 7l-1.12.32-.82 2a1 1 0 11-1.86-.74l1-2.5a1 1 0 01.66-.59l3.36-1h.12c.11-.014.22-.014.33 0h.1a2.16 2.16 0 012 1.51 2.56 2.56 0 00.84 1.394z"
      />
    </svg>
  )
}

export default SvgWalk
