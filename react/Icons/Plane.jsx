// Automatically created, please run `scripts/generate-svgr-icon.sh assets/icons/ui/plane.svg` to regenerate
import React from 'react'

import SvgPlaneTwake from './Plane_twake'
import { isTwakeTheme } from '../helpers/isTwakeTheme'

function SvgPlane(props) {
  if (isTwakeTheme()) return SvgPlaneTwake(props)
  return (
    <svg viewBox="0 0 16 16" {...props}>
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M15.539.461a1.574 1.574 0 00-2.25.023L9.88 4.081 2.025 1.135a.51.51 0 00-.54.117L.15 2.588a.51.51 0 00.108.804l6.561 3.75-3.57 3.57-1.74-.435a.51.51 0 00-.506.158l-.861.98a.51.51 0 00.18.805l2.417 1.042 1.042 2.416a.51.51 0 00.806.181l.979-.861a.51.51 0 00.158-.507l-.435-1.739 3.57-3.57 3.75 6.56a.51.51 0 00.804.108l1.336-1.335a.51.51 0 00.117-.54L11.918 6.12l3.597-3.41a1.576 1.576 0 00.023-2.25z"
      />
    </svg>
  )
}

export default SvgPlane
