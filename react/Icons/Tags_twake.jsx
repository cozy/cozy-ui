// Automatically created, please run `scripts/generate-svgr-icon.sh assets/icons/twake/permissions/tags_twake.svg` to regenerate;
import React from 'react'

function SvgTagsTwake(props) {
  return (
    <svg viewBox="0 0 16 16" fill="none" {...props}>
      <path
        d="M5 6a1 1 0 100-2 1 1 0 000 2z"
        fill="#1D212A"
        fillOpacity={0.9}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M2 0a2 2 0 00-2 2v5.89a2 2 0 00.586 1.415l5.98 5.973a2.462 2.462 0 003.481 0l5.236-5.237.002-.002a2.461 2.461 0 000-3.47L9.303.587A2 2 0 007.888 0H2zm0 2h5.888l5.978 5.98a.461.461 0 010 .649l-5.234 5.236a.46.46 0 01-.652 0L2 7.89V2z"
        fill="#1D212A"
        fillOpacity={0.9}
      />
    </svg>
  )
}

export default SvgTagsTwake
